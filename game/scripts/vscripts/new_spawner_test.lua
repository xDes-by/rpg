...
--герой зашел в зону
function OnHeroInZone(trigger) -- TODO: НЕ РАБОТАЕТ ЧЕРЕЗ КЛАСС (ВЫЗЫВАЕТСЯ ТОЛЬКО ТОГДА КОГДА ГЕРОЙ ЗАХОДИТ В ЗОНУ)
    ...
    if Spawner.active_zones[zone_name] == 0 then
        ...
        if not trigger[zone_name .. "_spawner_function"] then
            -- Spawner:StartSpawnZoneCreeps(trigger, zone_name)  -- TODO: ЧТО ЭТО ЗА ФУНКЦИЯ? ЕЕ ТУТ НЕТ
        else

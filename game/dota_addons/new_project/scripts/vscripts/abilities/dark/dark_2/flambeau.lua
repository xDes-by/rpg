LinkLuaModifier( "modifier_flambeau",  "abilities/dark/dark_2/flambeau", LUA_MODIFIER_MOTION_NONE )

flambeau = class({})

function flambeau:GetIntrinsicModifierName()
	return "modifier_flambeau"
end

-------------------------------------------------------------------------

modifier_flambeau = class({})

function modifier_flambeau:IsHidden()
	return true
end

function modifier_flambeau:CheckState()
	local state =
	{
...
function modifier_flambeau:OnCreated( kv )
	if IsServer() then
...
function modifier_flambeau:OnIntervalThink()
	if IsServer() then
		...
		if #enemies > 0 and not self.quest_trigger then
			...
			Quest:UpdateCounter('dark_2', 1)
			_G.dark_2.count = _G.dark_2.count + 1

ability_npc_boss_dark_1_spell4 = class({})

LinkLuaModifier( "modifier_ability_npc_boss_dark_1_spell4","abilities/dark/dark_1/ability_npc_boss_dark_1_spell4", LUA_MODIFIER_MOTION_NONE )

function ability_npc_boss_dark_1_spell4:OnSpellStart()
    ...
    self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_ability_npc_boss_dark_1_spell4", {duration = 0.5})
...
modifier_ability_npc_boss_dark_1_spell4 = class({})

function modifier_ability_npc_boss_dark_1_spell4:IsHidden()
    return false
end

function modifier_ability_npc_boss_dark_1_spell4:IsDebuff()
    return false
end

function modifier_ability_npc_boss_dark_1_spell4:IsPurgable()
    return false
end

function modifier_ability_npc_boss_dark_1_spell4:RemoveOnDeath()
    return true
end

function modifier_ability_npc_boss_dark_1_spell4:OnCreated()
	if not IsServer() then return end
	local damage = self:GetAbility():GetSpecialValueFor("damage")
    self.table = {victim = self:GetParent(),
    damage = self:GetCaster():GetBaseDamageMin() * damage,
...
function modifier_ability_npc_boss_dark_1_spell4:OnDestroy()
    if IsClient() then
        return
    end
...
function modifier_ability_npc_boss_dark_1_spell4:OnIntervalThink()

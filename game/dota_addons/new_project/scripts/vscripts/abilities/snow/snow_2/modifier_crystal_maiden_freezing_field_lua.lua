...
function modifier_crystal_maiden_freezing_field_lua:OnCreated( kv )
	...
	if self:GetCaster():FindAbilityByName("npc_crystal_maiden_3")~=nil then
		if self:GetCaster():FindAbilityByName("npc_crystal_maiden_3"):GetLevel() > 0 then 
			self.int_damage = self.int_damage + 0.1
		end
	end
	
	self.try_damage = self.explosion_damage + self:GetCaster():GetIntellect() * self.int_damage

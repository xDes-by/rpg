ability_npc_boss_cave1_spell1 = class({})

LinkLuaModifier( "modidfier_ability_npc_boss_cave1_spell1","abilities/cave1/ability_npc_boss_cave1_spell1", LUA_MODIFIER_MOTION_NONE )
LinkLuaModifier( "modidfier_ability_npc_boss_cave1_spell2","abilities/cave1/ability_npc_boss_cave1_spell2", LUA_MODIFIER_MOTION_NONE )

function ability_npc_boss_cave1_spell1:Precache(context)
...
function ability_npc_boss_cave1_spell1:OnSpellStart()
    local unit = CreateUnitByName("npc_boss_summon_cave1", self:GetCursorPosition(), true, nil, nil, DOTA_TEAM_BADGUYS )
    unit:AddNewModifier(self:GetCaster(), self, "modidfier_ability_npc_boss_cave1_spell1", {duration = 5})
    self:GetCaster():AddNewModifier(self:GetCaster(), self, "modidfier_ability_npc_boss_cave1_spell2", {})
end

modidfier_ability_npc_boss_cave1_spell1 = class({})
--Classifications template
function modidfier_ability_npc_boss_cave1_spell1:IsHidden()
    return true
end

function modidfier_ability_npc_boss_cave1_spell1:IsPurgable()
    return false
end

function modidfier_ability_npc_boss_cave1_spell1:DestroyOnExpire()
    return true
end

function modidfier_ability_npc_boss_cave1_spell1:OnCreated()
    self:StartIntervalThink(2)
    self:OnIntervalThink()
end

function modidfier_ability_npc_boss_cave1_spell1:OnDestroy()
    UTIL_Remove(self:GetParent())
end

function modidfier_ability_npc_boss_cave1_spell1:OnIntervalThink()
    local radius = 500
    local speed = 500

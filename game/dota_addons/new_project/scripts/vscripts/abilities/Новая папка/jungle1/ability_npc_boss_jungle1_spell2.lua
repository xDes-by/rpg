ability_npc_boss_jungle1_spell2 = class({})

LinkLuaModifier( "modifier_ability_npc_boss_jungle1_spell2", "abilities/jungle1/ability_npc_boss_jungle1_spell2", LUA_MODIFIER_MOTION_NONE )

function ability_npc_boss_jungle1_spell2:OnSpellStart()
    local pos = self:GetCaster():GetAbsOrigin()
    local waves = self:GetSpecialValueFor("waves")
    for i=1,waves do
        CreateModifierThinker(self:GetCaster(), self, "modifier_ability_npc_boss_jungle1_spell2", {duration = 1}, pos + RandomVector(500), self:GetCaster():GetTeamNumber(), false)
    end
end

modifier_ability_npc_boss_jungle1_spell2 = class({})

function modifier_ability_npc_boss_jungle1_spell2:OnCreated()
    if IsClient() then
        return
    end
...
function modifier_ability_npc_boss_jungle1_spell2:OnDestroy()
    if IsClient() then
        return
    end

...
function CosmeticLib:Init()
	if not CosmeticLib.bHasInitialized then
		...
		Convars:RegisterCommand( "print_items_by_hero_name", function( cmd, hero_name ) 
				return CosmeticLib:PrintSetsByHeroName( hero_name )
...
-- Print available set_id for hero to console
function CosmeticLib:PrintSetsByHeroName( hero_name )
	local hero_sets = CosmeticLib:GetAllSetsForHero( hero_name )
	print( "[CosmeticLib] Available set for " .. hero_name .. " are" )
	for _, item_id in pairs( hero_sets ) do
		print( "[CosmeticLib] Set ID: " .. item_id .. "\tName: " .. CosmeticLib._AllItemsByID[ item_id ][ "name" ] )
	end
end
	...
	if unit and hero_name and set_id and CosmeticLib:_Identify( unit ) then
		...
		if selected_item and CosmeticLib._SetByHeroes[ hero_name ]
			...
			for slot_name, item_id in pairs ( CosmeticLib._Sets[ selected_item[ "name" ] ] ) do
				--print("itemid = " .. item_id, "slotname = " .. slot_name)
...
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------
--ДОП ФУНКЦИИ НЕ ОТНОСЯЩИЕСЯ К БИБЛИОТЕКЕ
-----------------------------------------------------------------------------
-----------------------------------------------------------------------------